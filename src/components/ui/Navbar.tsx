...
export function Navbar() {
    return (
        <nav className=" px-2 md:px-8 md:mx-0 flex flex-row">
            <div className="w-full max-w-fit rounded-xl border border-border p-1 bg-white dark:bg-gray-800 dark:border-gray-700 transition-colors duration-300 mr-2"> {/* Added mr-2 */}
                        ...
                        {NAVIGATION_ITEMS.map(({ id, label, href }) => {
                            ...
                            return (
                                <Link
                                    ...
                                    className="group cursor-pointer px-2 sm:px-3 py-2 text-sm md:text-base transition-all duration-300 text-muted-foreground hover:text-foreground data-[checked=true]:text-foreground dark:text-white dark:hover:text-foreground bg-transparent dark:data-[checked=true]:text-foreground" // Changed px-3 to px-2 sm:px-3 and removed extra spaces/newlines
